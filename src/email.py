...
class email:
		...
		if "send an email to " not in input:
			...
			if hasmsg:
				...
				try:
					...
					contactemail = getcontactemail(contact.lower)
	...
	def getcontactemail(contact):
		searchterm = ' ' + contact + '=' 
		with open("contacts.txt", "w") as file:
			contactlist = file.readlines() 
			listplaceholder = 0
			while listplaceholder <= len(contactlist): 
				if searchterm in contactlist[listplaceholder]: 
					searchtermline = contactlist[listplaceholder]
					queriedemailaddress = searchtermline.split("=",1)[1] 
					return queriedemailaddress
